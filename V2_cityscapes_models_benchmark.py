...
def try_build_mask2former(device: torch.device) -> List[BaseAdapter]:
    ...
    class Mask2FormerAdapter(BaseAdapter):
        ...
        def predict(self, images_01: torch.Tensor):
            ...
            returns pred [B,H,W], conf [B,H,W]
            """
            B, _, H, W = images_01.shape

            # safer path: list of CPU tensors
            ...
            pred = torch.stack(seg_list, dim=0).to(self.device).to(torch.int64)

            # ⚠️ per consistency con gli altri modelli:
            # ricaviamo una "confidence" fittizia (uniforme)
...
def evaluate_all_models():
    ...
    adapters: List[BaseAdapter] = [
        ...
        DeepLabV3CityscapesAdapter(device),
    ]
    ...
    # -------------------------
    # Sanity check (one batch): DeepLabV3 predicted class histogram
    # -------------------------
    def quick_hist(pred_hw: torch.Tensor, num_classes=19):
        # pred_hw: [H,W] on any device
        h = torch.bincount(pred_hw.view(-1).to(torch.int64).cpu(), minlength=num_classes).float()
        return (h / h.sum().clamp_min(1.0)).numpy()

    # id2label from canonical (SegFormer)
    for k in [8, 10, 11, 13]:
        print("[sanity] class", k, "=", id2label[k] if isinstance(id2label, dict) and k in id2label else str(k))

    # take first batch only
    first_batch = next(iter(loader))
    pair0 = first_batch["pair"]
    clean0 = pair0[:, 0].to(device)
    adv0 = pair0[:, 1].to(device)

    # find DeepLab adapter
    dl = None
    for a in adapters:
        if "DeepLabV3" in a.name or "DeepLab" in a.name:
            dl = a
            break

    if dl is not None:
        pred_c, conf_c = dl.predict(clean0)
        pred_a, conf_a = dl.predict(adv0)

        hc = quick_hist(pred_c[0], num_classes=19)
        ha = quick_hist(pred_a[0], num_classes=19)

        print("[sanity] DeepLabV3 class histogram (clean, first image):", np.round(hc, 3))
        print("[sanity] DeepLabV3 class histogram (adv,   first image):", np.round(ha, 3))
        print("[sanity] DeepLabV3 mean conf (clean/adv):",
              float(conf_c[0].mean().item()), float(conf_a[0].mean().item()))
    else:
        print("[sanity] DeepLabV3 adapter not found, skipping.")

    # --------------------------------------------
    # Test different preprocessing variants
    # --------------------------------------------
    def dl_forward_variant(dl_model, images_01, variant: str):
        if variant == "imagenet":
        ...
        out = dl_model.model(x)["out"]
        probs = F.softmax(out, dim=1)
        conf = probs.max(dim=1).values
        pred = probs.argmax(dim=1)
        return pred, conf

    for variant in ["imagenet", "none", "255"]:
        pc, cc = dl_forward_variant(dl, clean0, variant)
        hc = quick_hist(pc[0], num_classes=19)
        print(f"[sanity] DeepLabV3 hist variant={variant}:",
              np.round(hc, 3),
              "mean_conf=", float(cc[0].mean().item()))
    ...
    for scen in PLOT_SCENARIOS:
        ...
        """NUOVO BARPLOT MA MANCA DEEPLAB NEL PLOT"""
        # Grouped barplot
        ...
        """ OLD FUNCTION LIKE PLOT
        # Line plot (function-like)
        fig = plt.figure(figsize=(12, 5))
        ax = fig.add_subplot(111)
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(list(piv.index), rotation=45, ha="right")
        ax.set_ylabel("Retention (masked)")
        ax.set_title(f"Class retention — {scen} (conf >= {CONF_THRESHOLD}, mass coverage >= {MASS_COVERAGE:.0%})")
        ax.legend(fontsize=8, ncol=2)
        fig.tight_layout()
        fig.savefig(os.path.join(plot_dir, f"plot_lines_class_retention__{scen}.png"), dpi=200)
        plt.close(fig)
        """
    ...
    for scen in PLOT_SCENARIOS:
        ...
        fig.tight_layout()
        #fig.savefig(os.path.join(qual_dir, f"qual_worst_{MODEL_FOR_QUAL}__{scen}.png"), dpi=200) #COn Deeplab
        fig.savefig(os.path.join(qual_dir, f"qual_segformer_worst__{scen}.png"), dpi=200)
        plt.close(fig)


        # ==========================================================
        # QUALITATIVE: DeepLabV3 clean vs adverse on the SAME worst cases
        # ==========================================================
        qual_dl_dir = os.path.join(SAVE_DIR, "qualitative_deeplabv3")
        os.makedirs(qual_dl_dir, exist_ok=True)

        # Find DeepLab adapter instance
        dl_vis = None
        for a in adapters:
            if "DeepLabV3" in a.name or "DeepLab" in a.name:
                dl_vis = a
        ...
        if dl_vis is None:
            print("[warn] DeepLabV3 adapter not found -> skipping DeepLab qualitative.")
        else:
            for scen in PLOT_SCENARIOS:
                worst_df = select_worst_cases(CSV_IMG, MODEL_FOR_QUAL, scen, k=N_QUAL_SAMPLES)
                if worst_df.empty:
                    print(
                        f"[warn] No worst cases found for {MODEL_FOR_QUAL} in {scen} -> skipping DeepLab qualitative.")
                    continue
                ...
                for r, row in enumerate(worst_df.itertuples(index=False)):
                    ...
                    pred_c, _ = dl_vis.predict(clean_t)
                    pred_a, _ = dl_vis.predict(adv_t)
                    ...
                    ax1 = fig.add_subplot(len(worst_df), 4, r * 4 + 2)
                    ax1.imshow(clean_mask);
                    ax1.set_title("DeepLab pred clean");
                    ax1.axis("off")
                    ...
                    ax3 = fig.add_subplot(len(worst_df), 4, r * 4 + 4)
                    ax3.imshow(adv_mask);
                    ax3.set_title("DeepLab pred adv");
                    ax3.axis("off")

                fig.tight_layout()
                fig.savefig(os.path.join(qual_dl_dir, f"qual_deeplab_clean_vs_{scen}.png"), dpi=200)
                plt.close(fig)
    ...
    #print(" - plots/plot_lines_class_retention__<scenario>.png  (3 plots)") #FUNCTION LIKE PLOT
